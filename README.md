...
- **原子化配置持久化 (v5.7.1)**: DataManager 采用 `NamedTemporaryFile` + `os.replace` 策略，确保 Windows 下进程崩溃也不会损坏配置文件。
- **强制进程治理 (v5.7.1)**: 实现外部工具 300s 强制超时暗杀 logic，配合 Unix 进程组清理，彻底解决句柄泄露与丧尸进程问题。
