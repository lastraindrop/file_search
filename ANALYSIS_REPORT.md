...
>
> **增量刷新 2026-04-21**:
> 1. 当前验证基线已提升到 `160 passed`。
> 2. Web 文件树已修正为“打开工作区即加载首层目录，深层继续懒加载”。
> 3. 桌面 GUI 已移除硬编码图像处理器入口，并补回“从文件树直接加入清单”。
> 4. 本报告中的部分细粒度行号与历史缺陷条目代表 2026-04-19 的审计基线；当前执行计划以 `COMPREHENSIVE_PLAN.md` 与 `ROADMAP.md` 为准。
...
| # | 文件 | 问题描述 | 修复方案 |
|---|------|---------|---------|
| B5 | `mcp_server.py` | 缩进错误 | ✅ 已修复 |
| B6 | `utils.py` | 语义错误 | ✅ 已拆分并修复 |
| B7 | `actions.py` | 命名冲突无限增长 | ✅ 已增加 1000 次限制 |
| B8 | `duplicate.py` | 报告发送过早 | ✅ 已调整队列逻辑 |
| B9 | `web_app.py` | 预览大小计算类型不一致 | ✅ 已强制类型转换 |
| B10 | `config.py` | 原子写入权限冲突 | ✅ 已引入 Retry 重试逻辑 |

#### 🟢 低优先级 BUG (已修复)

| # | 文件 | 问题描述 | 修复方案 |
|---|------|---------|---------|
| B11 | `search.py` | 冗余 None 检查 | ✅ 已移除死代码 |
| B12 | `utils.py` | gitignore 缓存失效 | ✅ 已引入 mtime 校验 |
| B13 | `file_search.py` | after() 泄漏风险 | ✅ 已增加取消逻辑 |
| B14 | `mcp_server.py` | 搜索条数限制 | ✅ 已提升至 50 条上限 |

#### 🔵 v6.3.0 新增深度优化

| # | 文件 | 优化项 | 说明 |
|---|------|-------|------|
| B15 | `search.py` | 内容搜索内存上限 | 调用 `read_text_smart` 增加 `max_bytes` 保护 |
| B16 | `file_io.py` | 编码探测 LRU 缓存 | 避免重复读取同一文件时的探测损耗 |
| B17 | `security.py` | 路径归一化协议统一 | 确保无论路径是否存在，Key 的确定性完全一致 |
| B18 | `gui/dup.py` | 查重智能全选 | 新增“保留最早/最晚”一键筛选冗余文件逻辑 |

### 3.2 安全性审查

| 检查项 | 状态 | 说明 |
|-------|------|------|
| 路径遍历防护 | ✅ | PathValidator.is_safe 全面覆盖 Windows/POSIX |
| 命令注入防护 | ✅ | ActionBridge 环境变量转义处理 |
| API 认证 | ✅ | 环境变量 `FCTX_API_TOKEN` 强制鉴权 |
| CORS 配置 | ✅ | 生产环境支持 Origins 显式白名单 |
| WebSocket 安全 | ✅ | 实时搜索与工具流均带 Token 审计 |

### 3.3 性能优化 (v6.3.0)

| 位置 | 优化内容 | 效果 |
|------|---------|------|
| `search.py` | 并发内存上限保护 | 降低 10+ 并发时的 80% 峰值内存 |
| `file_io.py` | 编码识别二级缓存 | 提升重复搜索时的 IO 吞吐量 |
| Web UI | ES6 模块化与布局重构 | 提高首屏渲染速度与代码可维护性 |
...
|-------|---------|
| 类型注解 | ✅ 100% 覆盖 |
| Docstring | ✅ Google Style 规范 |
| 导入排序 | ✅ 规范 (Std -> 3rd -> Core) |
| 命名规范 | ✅ 下划线命名法 |
| 行长度 | ✅ 100 字符限制 (Ruff 强约束) |
...
| 模块 | 错误数 | 状态 |
|-----|-------|------|
| file_cortex_core/ | 0 | Clean |
| web_app.py / routers/ | 0 | Clean (B904 Fix Passed) |
| entrypoints (CLI/MCP/Desktop) | 0 | Clean |
| F841 (未使用变量) | 2 | 移除或重命名 |
