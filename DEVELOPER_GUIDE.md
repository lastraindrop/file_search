...
│  │(Tkinter/GUI)│  │(FastAPI/ES6)│  │ (fctx.py)  │  │ Server │ │
│  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘  └───┬────┘ │
└─────────┼────────────────┼────────────────┼─────────────┼─────────────┘
          │ (Modular GUI)  │ (ES6 Modules)  │             │
          ▼                ▼                ▼             ▼
┌─────────────────────────────────────────────────────────────────┐
...
- **逻辑动态对齐**: 类似于预览上限、Token 预算、搜索默认参数等必须从 `global_settings` 或项目 `search_settings` 动态读取。后端使用 `routers.schemas` 进行严格校验，前端通过 `state.js` 统一维护。
- **交互入口一致**: 桌面 GUI、Web UI 与 CLI 的高频操作必须保持同等级可达性；例如“加入清单”不能只存在于某一个入口或依赖收藏中转。
- **依赖注入模式**: 后端路由全面采用 `Depends(get_dm)`，确保 DataManager 的生命周期由应用容器管理，提升并发稳定性。
