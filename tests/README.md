...
> **测试数**: 846 | **状态**: All Passed | **Ruff**: 0 errors | **版本**: 6.6.0

本项目包含 **846** 项核心全自动化的 `pytest` 测试，采用 **领域驱动深度加固 (Domain-Driven Hardening)** 架构，实现了从底层 IO 到上层 API 契约、前端契约、CLI 与 MCP 的全方位覆盖。
...
| **v6.5.0 安全修复** | `test_security_fixes_v650.py` | 39 | 符号链接遍历/访问控制/MCP安全/线程安全/ProcessManager/死代码消除 |
| **v6.5.0 前端契约** | `test_frontend_contract.py` | 43 | HTML结构/JS模块/CSS/前端功能契约/copy-extract Web wiring/bulk copy-extract+progress/CSP 事件委托/主题/布局/虚拟滚动/CSP Header/Style 契约/轮询可取消 |
| **v6.5.0 Web API (合并)** | `test_web_api.py` | 83 | CRUD/CORS/Auth/设置/WebSocket/文件操作/安全/端点契约/归档安全/WS fallback/copy/extract/批量copy+progress API |
...
| **搜索引擎** | `test_search_engine.py` | 23 | 搜索矩阵 (4 mode × 2 params) + 共享线程池恢复回归 |
| **安全沙盒** | `test_security_resilience.py` | 23 | 路径验证器全矩阵 (15 场景) |
| **文件操作** | `test_fileops_advanced.py` | 9 | 文件操作完整覆盖 |
| **配置管理** | `test_dm_config.py` | 10 | DataManager 持久化/并发/分类遍历校验 |
| **集成** | `test_core_integration.py` + `test_scenarios.py` + `test_ai_enhanced.py` | 20 | 编码回弹/级联回滚/完整流程/路径收集 |
| **上下文/MCP** | `test_context_formatter.py` + `test_mcp_server.py` | 10 | XML/MD 导出/MCP 协议 |
| **格式化** | `test_utils_format.py` | 9 | 格式化/Token估算/路径分隔符契约 |
