...
- [x] **快速路径搜集 (Path Collection)**: 实现多端一致的自定义路径格式化搜集。
- [x] **工作区配置管理 (v5.3+)**: 实现置顶项目 (Pin) 与 LRU 历史记录，确保每项目配置动态对齐。

## 📍 阶段 2：高级 AI 编排与增强 (v5.4 短期目标)
- [ ] **ActionBridge 进程管控**: UI 支持手动终止长时脚本运行（Kill 信号发送）。
- [ ] **增强文件预览**: 支持 Markdown 渲染、图片预览及长文本智能截断预览。
- [ ] **智能上下文清洗**: 暂存清单自动检测重复代码段或冗余文件的重叠 Token。

## 📍 阶段 3：多端同步与插件化 (v5.5 中期目标)
- [ ] **配置云同步 (Encrypted)**: 支持加密同步配置至 GitHub Gist 或 WebDAV。
- [ ] **插件系统 (UI Hooks)**: 支持加载自定义 JS 插件以扩展右侧 Action 按钮。

## 🚀 长期愿景 (v6.0+)
- [ ] **智能上下文摘要 (AI-Driven)**: 引入轻量级本地模型（如 Ollama）对收集到的文件内容进行自动摘要。
- [ ] **远程服务器编排**: 支持通过 SSH/SFTP 挂载远程目录。
- [ ] **多模态编排**: 支持 PDF 文档、Word 文档的上下文解析与一键导出。
